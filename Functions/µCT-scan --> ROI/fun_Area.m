...
allNodeIndices = {nodeIndicesXMax, nodeIndicesXMin, nodeIndicesYMax, nodeIndicesYMin, nodeIndicesZMax, nodeIndicesZMin};
sideName = {'Right', 'left','Top','Bottom','Front','Back'};
facesForSides = struct('Faces', [], 'FaceVertices', []);% Initialize a structure array to store face information for each side
totalAreas = struct('Side', [], 'Area', []); 
sideColors = {'r', 'g', 'b', 'c', 'm', 'y'}; Area.side_colors = sideColors;% Colors for each side
Side_name = {'Right', 'Left', 'Top', 'Bottom', 'Front', 'Back'};
...
for sideIndex = 1:numel(allNodeIndices)
    ...
    disp(['Total area of ' sideName{sideIndex} ' side : ' num2str(totalArea)]);
