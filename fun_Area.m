...
Area = struct(); % Create a structure to store the output data
...
allNodeIndices = {nodeIndicesXMax, nodeIndicesXMin, nodeIndicesYMax, nodeIndicesYMin, nodeIndicesZMax, nodeIndicesZMin}; % Store the indices of the nodes located on the faces of interest 
facesForSides = struct('Faces', [], 'FaceVertices', []);% Initialize a structure array to store face information for each side
totalAreas = struct('Side', [], 'Area', []); % Initialize a structure array to store Area informations for each face 
sideColors = {'r', 'g', 'b', 'c', 'm', 'y'}; Area.side_colors = sideColors;% Colors for each side
Side_name = {'Right', 'Left', 'Top', 'Bottom', 'Front', 'Back'}; % Name of the faces 
...
for i = 1:numFaces
    ...
    area = 0.5 * norm(cross(vertex2 - vertex1, vertex3 - vertex1));
